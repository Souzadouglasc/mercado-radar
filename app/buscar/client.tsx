  ...
  useEffect(() => {
    ...
    async function fetchResults() {
      ...
      try {
        ...
        const data = await searchProducts(supabase, q, 50, city);
...
export default function BuscarClient({
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      >
        <Resultados key={`${q}:${cityFilter ?? "all"}:${groupEnabled}:${sortValue}`} q={q} city={cityFilter} group={groupEnabled} sort={sortValue} />
      </Suspense>
    </div>
  );
}
