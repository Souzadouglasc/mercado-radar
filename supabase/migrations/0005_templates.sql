-- MercadoRadar - Tarefa I2: Templates de lista de compras
-- Tabelas para modelos de lista + itens por NOME (não FK)
-- custom_name em shopping_list_items para itens avulsos
...
create trigger list_templates_updated_at before update on list_templates for each row execute function set_updated_at();

-- Itens do template (por nome/brand/quantity/unit, não FK)
...
-- Coluna para itens avulsos na lista (nome custom quando não achou produto)
...
-- Templates: SELECT público (qualquer um pode usar modelos)
...
-- Itens do template: SELECT público
...
-- Seed: 4 modelos brasileiros típicos
